...
class PopulationGenerator:
    ...
    def _draw_children(self, children_count: int, household_idx: int, current_index: int) -> Tuple[List[BasicNode], int]:
    ...
    def run(self, simulation_folder: Optional[Path] = None):
        ...
        for idx in tqdm(range(self.number_of_households)):
            ...
            nodes.extend(adults)
            households[idx] = [child.idx for child in children] + [adult.idx for adult in adults]

            if idx % 1000 == 0:
                hdf = pd.DataFrame(data={'household_idx': list(households.keys()), 'idx': list(households.values())})
                pdf = nodes_to_dataframe(nodes)
                if idx == 1000:
                    pdf.to_csv(str(simulation_folder / 'population.csv'), index=False)
                    hdf.to_csv(str(simulation_folder / 'households.csv'), index=False)
                else:
